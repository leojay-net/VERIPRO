...
async fn main() {
    ...
    // Use port 3001 to avoid conflict with Next.js (3000)
...
fn find_forge_std_path() -> Option<String> {
    ...
    let common_paths: Vec<String> = vec![
        ...
        format!("{}/Desktop/CODE/mantle/VeriPro/test-contract/lib/forge-std", home),
        format!("{}/Desktop/CODE/mantle/VeriPro/smart-contracts/lib/forge-std", home),
    ...
    println!("Could not find forge-std in any of {} locations", common_paths.len());
...
async fn start_verification(
    ...
    if payload.spec_source.trim().is_empty() {
        return error_response(
            job_id,
            "No specification provided. Please add a specification file with test functions.".to_string()
        );
    }
    ...
    println!("Contract source (first 200 chars): {}", 
             payload.contract_source.chars().take(200).collect::<String>());
    println!("Spec source (first 200 chars): {}", 
    ...
    if let Some(forge_std_path) = find_forge_std_path() {
        ...
        if let Err(e) = symlink(&forge_std_path, &forge_std_link) {
            println!("Warning: Failed to symlink forge-std: {} - will try forge install", e);
        } else {
...
"#;
    ...
    println!("Running prover on {} with test contract {}", work_dir, test_contract_name);
